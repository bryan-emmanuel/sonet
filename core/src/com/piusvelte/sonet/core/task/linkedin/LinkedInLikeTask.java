...
public class LinkedInLikeTask extends LikeTask {

	private static final String TAG = "LinkedInLikeTask";

	public LinkedInLikeTask(SonetComments activity, long accountId) {
	...
	protected String doInBackground(String... params) {
		...
		if (account.moveToFirst()) {
			...
			try {
				HttpPut httpPut = new HttpPut(String.format(LINKEDIN_IS_LIKED, LINKEDIN_BASE_URL, params[ID]));
				httpPut.addHeader(new BasicHeader("Content-Type", "application/xml"));
				if (Boolean.parseBoolean(params[LIKE])) {
					httpPut.setEntity(new StringEntity(String.format(LINKEDIN_LIKE_BODY, true)));
					if (SonetHttpClient.httpResponse(httpClient, sonetOAuth.getSignedRequest(httpPut)) != null) {
						...
						result = activity.getString(R.string.like);
					}
				} else {
					httpPut.setEntity(new StringEntity(String.format(LINKEDIN_LIKE_BODY, false)));
					if (SonetHttpClient.httpResponse(httpClient, sonetOAuth.getSignedRequest(httpPut)) != null) {
						publishProgress(Sonet.getServiceName(activity.getResources(), Sonet.LINKEDIN), activity.getString(R.string.success));
						result = activity.getString(R.string.like);
					} else {
